...
public class Delaunay {
	...
	public List<Point[]> computeTriangles() {
		List<Point[]> triangles = new LinkedList<Point[]>();
	...
	public LinkedList<Point[]> copieTriangle(List<Point[]> triangles,ArrayList<Point> points){
		LinkedList<Point[]> tr=new LinkedList<Point[]>();
		
		if(triangles.size()!=0){
		for(int i=0;i<triangles.size();i++){
			if(triangles.get(i)[0].idAppartient(points)
			 &&triangles.get(i)[1].idAppartient(points) 
			 &&triangles.get(i)[2].idAppartient(points) ){
				//System.out.println("appartient!");
				Point[] p=new Point[3];
				p[0]=points.get(triangles.get(i)[0].getId()-1);
				p[1]=points.get(triangles.get(i)[1].getId()-1);
				p[2]=points.get(triangles.get(i)[2].getId()-1);
				
				tr.add(p);
			}
			//System.out.print("triangle1:"); afficherTriangle(triangles.get(i));
			//System.out.print("triangle2:"); afficherTriangle(tr.get(i));
			//System.out.println();
		}
		}
		return tr;
